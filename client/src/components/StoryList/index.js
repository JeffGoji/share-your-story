...
const StoriesList = ({ stories }) => {
  ...
  const handleDeleteStory = async (storyId) => {
    ...
    if (!stories?.length) {
      return (
        <h3 className="text-center text-white bg-dark rounded">
          No Stories Yet
        </h3>
      );
    }
  };
  //   const handleUpdateStory = async (storyId, storyText) => {
  //           const token = Auth.loggedIn() ? Auth.getToken() : null;
  //           if (!token) {
  //             return false;
  //           }
                      ...
                      <form>
                        {loggedIn && (
                          <>
                            ...
                            <button
                              type="button"
                              className="btn btn-warning btn-sm px-4 mt-2 m-1"
                            >
                              Edit Story
                            </button>
                          </>
